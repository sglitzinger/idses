'''
Computes an integrated crown schedule as well as a device design (number of big and LITTLE cores) from a task set, a given deadline and a chip area budget.
'''
...
# Arguments to be passed: path to task set file, path to results directory, task set file
def main():
    ...
    if opt_model.SolCount >= 1:
        ...
        with open(results_filename, 'a') as energyfile:
            if os.path.getsize(results_filename) == 0:
                energyfile.write("taskset,deadline,#big,#little,energy_consumption,objective function value\n")
            energyfile.write(os.path.splitext(task_filename)[0] + "," + str(deadline) + "," + str(numbig) + "," + str(numlitte) + "," + str(energy) + "," + str(opt_model.objVal) + "\n")
        ...
        with open(results_filename, 'a') as energyfile:
            if os.path.getsize(results_filename) == 0:
                energyfile.write("taskset,deadline,#big,#little,energy_consumption\n")
            energyfile.write(os.path.splitext(task_filename)[0] + "," + str(deadline) + "," + "\n")
