'''
Computes an integrated crown schedule as well as a device design (number of big and LITTLE cores) from a task set, a given deadline and a chip area budget, which may be set via the command line.
'''
...
# in order: MEMORY, BRANCH, FMULT, SIMD, MATMUL
execution_time_multipliers = [0.676, 1.376, 0.264, 0.278, 0.746]
# Per core power values for different task types and frequency levels for big cores
powers_big = [
...
# Per core power values for different task types and frequency levels for LITTLE cores
powers_little = [
...
base_power_little = 0.645 # 2.58
base_power_big = 0.745 # 2.98
zeta = 1.0 # 0.649
kappa = 52.64
eta = 0.5
alpha = 3.0
deadline_factor = 0.6 # Adjust deadline
...
# def dyn_power(tasktype, group, freqlevel):
#     if is_little(group):
#         return powers_little[get_tasktype_index(tasktype)][freqlevel]
#     return powers_big[get_tasktype_index(tasktype)][freqlevel]

# def stat_power(f):
#     return f + kappa * min(freqs)

# def total_power(tasktype, group, freqlevel):
#     return zeta * dyn_power(tasktype, group, freqlevel) + (1 - zeta) * stat_power(freqs[freqlevel])

def power_little(tasktype, freqlevel):
    return powers_little[get_tasktype_index(tasktype)][freqlevel]

def power_big(tasktype, freqlevel):
    return powers_big[get_tasktype_index(tasktype)][freqlevel]
...
#         return tau / (tau + procs_alloc * psi)
#     return 0.000001
...
#         return 1.0
#     if procs_alloc > 1 and procs_alloc <= max_width:
#         return 1.0 - R * procs_alloc**2 / max_width**2 #tau / (tau + procs_alloc * psi)
#     return 0.000001
...
# def core_eff(tasktype, group):
#     if is_little(group):
#         # task is executed on LITTLE core
#         return 1.0
#     return execution_time_multipliers[get_tasktype_index(tasktype)]

def core_eff(tasktype):
    return execution_time_multipliers[get_tasktype_index(tasktype)]

# def is_little(group):
#     if group in [2,4,5,8,9,10,11]:
#         #return True
#         return False
#     #return False
#     return True
...
# def create_task_data(tasks, procs):
#     workloads = [None] * tasks
#     max_widths = [None] * tasks
#     psi_values = [None] * tasks
#     for i in range(tasks):
#         workloads[i] = random.randint(1,100)
#         max_widths[i] = random.randint(1,procs)
#         psi_values[i] = random.random()
#     return workloads, max_widths, psi_values
...
def compute_deadline(df, cores):
    ...
    lower_bound = total_workload / (cores * freqs[len(freqs)-1])
    print("Lower bound:", lower_bound)
    # upper_target = 2 * total_workload / (cores * freqs[0])
    upper_target = total_workload / (cores * freqs[0])
    # upper_target = lower_bound
    print("Upper target:", upper_target)
    return (lower_bound + upper_target) / 2

# def get_deadline(wloads, maxws, psis):
#     makespan_low_freq = 0
#     makespan_high_freq = 0
#     for i in range(len(wloads)):
#         runtime_low_freq = wloads[i] / (min(freqs) * maxws[i] * par_eff(wloads[i], psis[i], maxws[i], maxws[i]))
#         runtime_high_freq = wloads[i] / (max(freqs) * maxws[i] * par_eff(wloads[i], psis[i], maxws[i], maxws[i]))
#         makespan_low_freq += runtime_low_freq
#         makespan_high_freq += runtime_high_freq
#     return (makespan_low_freq + makespan_high_freq) // 2
...
def get_chip_size(numbig, numlitte):
    return numbig * area_big + numlitte * area_little
...
def main():
    ...
    maxcores = math.ceil((chip_area * (1-delta)) / area_little)
    ...
    set_K = range(0, num_freqs)
    set_T = range(0,2) # 2 core types (0 is big, 1 is LITTLE)
    ...
    # Number of big and LITTLE cores, respectively
    pb_var = opt_model.addVar(vtype=grb.GRB.INTEGER, name="pb")
    pl_var = opt_model.addVar(vtype=grb.GRB.INTEGER, name="pl")
    ...
    for j in set_J:
        opt_model.addConstr(
            lhs=grb.quicksum(x_vars[i,j,k,t] for i in set_I for k in set_K for t in set_T),
            sense=grb.GRB.EQUAL,
            rhs=1, 
    ...
    # print(get_deadline(workloads, max_widths, psi_values))
    ...
    for m in set_M:
        opt_model.addConstr(
            ...
            lhs=grb.quicksum(x_vars[i,j,k,0] * ((workloads[j] * core_eff(tasktypes[j])) / (get_group_width(i, cores) * freqs[k] * par_eff(workloads[j], psi_values[j], max_widths[j], get_group_width(i, cores)))) for i in get_groups(m, cores) for j in set_J for k in set_K) + grb.quicksum(x_vars[i,j,k,1] * (workloads[j] / (get_group_width(i, cores) * freqs[k] * par_eff(workloads[j], psi_values[j], max_widths[j], get_group_width(i, cores)))) for i in get_groups(m, cores) for j in set_J for k in set_K),
    ...
    # Constraint: Number of big and LITTLE cores must not lead to excessive chip area usage
    opt_model.addConstr(
        lhs=pb_var * area_big + pl_var * area_little,
        sense=grb.GRB.LESS_EQUAL,
        rhs=chip_area * (1-delta), 
        name="constraint_chip_area"
    )

    # # Constraint: Number of big and LITTLE cores must not use more chip area than 4+4 big.LITTLE CPU
    # opt_model.addConstr(
    #     lhs=pb_var * c + pl_var,
    #     sense=grb.GRB.LESS_EQUAL,
    #     rhs=4 * (1 + c), 
    #     name="constraint_chip_composition"
    # )

    ################################################################
    # Core constraint for experiments    
    ################################################################

    # # Constraint: total number of cores must not exceed cores
    # opt_model.addConstr(
    #     lhs=pb_var + pl_var,
    #     sense=grb.GRB.LESS_EQUAL,
    #     rhs=cores, 
    #     name="constraint_numcores"
    # )

    ################################################################
    # MAPPING CONSTRAINTS BIG
    ################################################################
    
    # Constraint: force ex_i,b to 1 if max(i) >= pb
    for i in set_I:
        opt_model.addConstr(
            lhs=get_max_index(i, cores) - ex_vars[i,0] * cores,
            sense=grb.GRB.LESS_EQUAL,
            rhs=pb_var,
            name="constraint_pb_ex_{0}".format(i)
        )

    # Constraint: force ex_i,b to 0 if max(i) < pb
    for i in set_I:
        opt_model.addConstr(
            lhs=get_max_index(i, cores) + (1 - ex_vars[i,0]) * cores,
            sense=grb.GRB.GREATER_EQUAL,
            rhs=pb_var + 1,
            name="constraint_pb_notex_{0}".format(i)
        )
    ...
    for i in set_I:
        for j in set_J:
            opt_model.addConstr(
                lhs=grb.quicksum(x_vars[i,j,k,0] for k in set_K),
                sense=grb.GRB.LESS_EQUAL,
                rhs=1 - ex_vars[i,0],
                name="constraint_pb_sched_{0}".format(i)
            )

    # for j in set_J:
    #     opt_model.addConstr(
    #         lhs=grb.quicksum(x_vars[i,j,k,0] for i in set_I for k in set_K),
    #         sense=grb.GRB.EQUAL,
    #         rhs=0, 
    #         name="constraint_mapped_not_big_{0}".format(j)
    #     )

    # ################################################################
    # # MAPPING CONSTRAINTS LITTLE
    # ################################################################

    # Constraint: force ex_i,l to 1 if min(i) < cores - pl + 1
    for i in set_I:
        opt_model.addConstr(
            lhs=get_min_index(i, cores) + ex_vars[i,1] * cores,
            sense=grb.GRB.GREATER_EQUAL,
            rhs=cores - pl_var + 1,
            name="constraint_pl_ex_{0}".format(i)
        )

    # Constraint: force ex_i,l to 0 if min(i) >= cores - pl + 1
    for i in set_I:
        opt_model.addConstr(
            lhs=get_min_index(i, cores) - (1 - ex_vars[i,1]) * cores,
            sense=grb.GRB.LESS_EQUAL,
            rhs=cores - pl_var,
            name="constraint_pl_notex_{0}".format(i)
        )
    
    # Constraint: do not allow scheduling to groups with ex_i,l = 1
    for i in set_I:
        for j in set_J:
            opt_model.addConstr(
                lhs=grb.quicksum(x_vars[i,j,k,1] for k in set_K),
                sense=grb.GRB.LESS_EQUAL,
                rhs=1 - ex_vars[i,1],
                name="constraint_pl_sched_{0}".format(i)
            )

    # for j in set_J:
    #     opt_model.addConstr(
    #         lhs=grb.quicksum(x_vars[i,j,k,1] for i in set_I for k in set_K),
    #         sense=grb.GRB.EQUAL,
    #         rhs=0, 
    #         name="constraint_mapped_not_little_{0}".format(j)
    #     )


    # Objective function: energy
    # objective = grb.quicksum(x_vars[i,j,k] * ((workloads[i] * total_power(freqs[k])) / (get_group_width(j, cores) * freqs[k] * par_eff(workloads[i], psi_values[i], max_widths[i], get_group_width(j, cores)))) for i in set_I for j in set_J for k in set_K)
    #objective = grb.quicksum(x_vars[i,j,k,t] * ((workloads[j] * core_eff(tasktypes[j], i) * total_power(tasktypes[j], i, k)) / (freqs[k] * par_eff(workloads[j], psi_values[j], max_widths[j], get_group_width(i, cores)))) for i in set_I for j in set_J for k in set_K for t in set_T)

    # No base power, no core minimization term
    #objective = grb.quicksum(x_vars[i,j,k,0] * ((workloads[j] * core_eff(tasktypes[j]) * power_big(tasktypes[j], k)) / (freqs[k] * par_eff(workloads[j], psi_values[j], max_widths[j], get_group_width(i, cores)))) for i in set_I for j in set_J for k in set_K) + grb.quicksum(x_vars[i,j,k,1] * ((workloads[j] * power_little(tasktypes[j], k)) / (freqs[k] * par_eff(workloads[j], psi_values[j], max_widths[j], get_group_width(i, cores)))) for i in set_I for j in set_J for k in set_K)
    
    # No base power, core minimization term
    #objective = grb.quicksum(x_vars[i,j,k,0] * ((workloads[j] * core_eff(tasktypes[j]) * power_big(tasktypes[j], k)) / (freqs[k] * par_eff(workloads[j], psi_values[j], max_widths[j], get_group_width(i, cores)))) for i in set_I for j in set_J for k in set_K) + grb.quicksum(x_vars[i,j,k,1] * ((workloads[j] * power_little(tasktypes[j], k)) / (freqs[k] * par_eff(workloads[j], psi_values[j], max_widths[j], get_group_width(i, cores)))) for i in set_I for j in set_J for k in set_K) + pweight * (pb_var + pl_var)
    
    # Base power, no core minimization term
    objective = grb.quicksum(x_vars[i,j,k,0] * ((workloads[j] * core_eff(tasktypes[j]) * power_big(tasktypes[j], k)) / (freqs[k] * par_eff(workloads[j], psi_values[j], max_widths[j], get_group_width(i, cores)))) for i in set_I for j in set_J for k in set_K) + grb.quicksum(x_vars[i,j,k,1] * ((workloads[j] * power_little(tasktypes[j], k)) / (freqs[k] * par_eff(workloads[j], psi_values[j], max_widths[j], get_group_width(i, cores)))) for i in set_I for j in set_J for k in set_K) + pl_var * base_power_little * deadline + pb_var * base_power_big * deadline
    
    # Base power, core minimization (should deliver same results as base power, no core minimization term)
    #objective = grb.quicksum(x_vars[i,j,k,0] * ((workloads[j] * core_eff(tasktypes[j]) * power_big(tasktypes[j], k)) / (freqs[k] * par_eff(workloads[j], psi_values[j], max_widths[j], get_group_width(i, cores)))) for i in set_I for j in set_J for k in set_K) + grb.quicksum(x_vars[i,j,k,1] * ((workloads[j] * power_little(tasktypes[j], k)) / (freqs[k] * par_eff(workloads[j], psi_values[j], max_widths[j], get_group_width(i, cores)))) for i in set_I for j in set_J for k in set_K) + pl_var * base_power_little * deadline + pb_var * base_power_big * deadline + pweight * (pb_var + pl_var)
    ...
    if opt_model.SolCount >= 1:
        ...
        numbig = opt_model.getVarByName("pb").X
        numlitte = opt_model.getVarByName("pl").X
        print("Number of big cores:", numbig)
        print("Number of LITTLE cores:", numlitte)
        ...
        opt_df_exp.to_csv(os.path.join(output_path, filename), index=False, columns=["task", "group", "frequency"])

        energy = opt_model.objVal - pweight * (numbig + numlitte)

        with open(results_filename, 'a') as energyfile:
            if os.path.getsize(results_filename) == 0:
                energyfile.write("taskset,deadline,max. chip size,actual chip size,#big,#little,energy_consumption,objective function value\n")
            energyfile.write(os.path.splitext(task_filename)[0] + "," + str(deadline) + "," + str(chip_area) + "," + str(get_chip_size(numbig, numlitte)) + "," + str(numbig) + "," + str(numlitte) + "," + str(energy) + "," + str(opt_model.objVal) + "\n")
        ...
        with open(results_filename, 'a') as energyfile:
            if os.path.getsize(results_filename) == 0:
                energyfile.write("taskset,deadline,chip size,#big,#little,energy_consumption,objective function value\n")
